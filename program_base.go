...
//go:generate go run ./cmd/bpf_enum_gen -header /usr/include/linux/bpf.h -enum bpf_prog_type -type ProgramType -pkg goebpf -out program_type.gen.go
//go:generate go run ./cmd/bpf_enum_gen -header /usr/include/linux/bpf.h -enum bpf_attach_type -type AttachType -pkg goebpf -out attach_type.gen.go
